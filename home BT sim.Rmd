...
namesb<-c("Ta1","Ta2","Ta3","Ta4","Ta5","Ta6","Ta7","Ta8","a9","Taten","Tb1","Tb2","Tb3","Tb4","Tb5","Tb6","Tb7","Tb8","Tb9","Home") #defining the names for each team except the final team as well as the name "home" for later use as well
...
Next we define the Bradley Terry model. We will use the same one for all different versions of the Bradley-Terry model, but with home/draw parameters being 1 if they are not included. In this model we have that the draw parameter=1 but the home parameter does not.
...
BTM<-function(i,j,theta){
  ...
  h<-theta[(tn+1)]# 1 if not included
...
```{r}
theta<-c(seq(0.05,1,0.05),1.5)
Ttheta<-c(seq(0.05,0.95,0.05),1.5)
dtfr<-allsim(theta)
dtfr
```
...
HLL<-function(theta){
  ...
  for (i in 1:tn){
    ...
    for (j in 1:tn){
      ...
      parta<-aij*I+bij*J
      partb<-aij*H
      partc<-(aij+bij)*logSumExp(c(H+I,J))
      ans<-ans+parta+partb-partc
    }
  }
  return(ans)
}
...
I then use optim to find the maximum likelihood estimators. The lower limit for the home advantage parameter is set approximately to zero. This is because we need $\gamma>1$ which is equivalent to $ln(\gamma)>0$.

```{r}
a<-c(rep(0.1,19),1.5)
lowlim<-rep(-100,20)
strengthH<-optim(theta <- a, HLL, method="L-BFGS-B",control=list(fnscale=-1),hessian=TRUE)
strengthH
```
...
```{r}
fisher_info<-solve(-strengthH$hessian)
prop_sigma<-sqrt(diag(fisher_info))
upper<-strengthH$par+1.96*prop_sigma
lower<-strengthH$par-1.96*prop_sigma
interval<-(data.frame(Team=namesb,value=exp(strengthH$par), upper=exp(upper), lower=exp(lower), True=Ttheta))
interval
```
